...
if [ -d "$PREFIX/bin" ]; then
    ...
    # Also support 'gemini' as an alternate alias
    ln -sf "$SCRIPT_DIR/termux_start.sh" "$PREFIX/bin/gemini" 2>/dev/null || true
fi
...
if ! grep -q "alias gemini=" "$HOME/.bashrc" 2>/dev/null; then
    echo "alias gemini='$SCRIPT_DIR/termux_start.sh'" >> "$HOME/.bashrc"
fi
